...
public class User {
	private static final String tableName = " tbl_user ";

	public boolean isExistUser(String username, String passwd) {
		List<Record> users = Db.find(
				"select * from " + tableName + " where username = '" + username + "' and password = '" + passwd + "'");
	...
	public Page<Record> getUsersByPagin(int page, int pageSize, int deleted, String orderCase) {
		return Db.paginate(page, pageSize, "select * ",
				"from " + tableName + " where deleted = ? " + (orderCase.equals("") ? "" : ("order by " + orderCase)),
				deleted);
	}
